...
def mlm(tensor, mask_token_id, mask_probability=1.0):
    # original_tensor = tensor.clone()
    sep_idxs = (tensor == 2).nonzero()[:,1]

    second_last_sep = sep_idxs[-2]
    last_sep = sep_idxs[-1]

    # rand = torch.rand(tensor.shape) #[0,1]
    # find the </s> token index and mask tokens after the last </s> token
    # mask_arr = (rand < 0.15) * (tensor > 2) 
    mask_arr = (tensor > 2) * (torch.arange(tensor.shape[1]) > second_last_sep) * (torch.arange(tensor.shape[1]) <= last_sep)
    for i in range(tensor.shape[0]):
        selection = torch.flatten(mask_arr[i].nonzero())
        num_tokens_to_mask = int(len(selection) * mask_probability)
        tokens_to_mask = random.sample(selection.tolist(), num_tokens_to_mask)
        tensor[i, tokens_to_mask] = mask_token_id #tokenizer.mask_token_id
    # breakpoint()
    return tensor 
...
def run_pretraining(df_train, df_val, params, model, tokenizer, device, run_name):
    ...
    scheduler = ReduceLROnPlateau(optim, 'min', patience=4, factor=0.5)
    best_loss = 999
...
if __name__ == '__main__':
    ...
    ################### temporary ###################
    df_train.rename(columns={'string_integrated2': 'text'}, inplace=True)
    df_val.rename(columns={'string_integrated2': 'text'}, inplace=True)
    ...
    # ================= 5. Run pretraining ======================
    run_name = '500k-sole-nsp-pt'+datetime.now().strftime("_%m%d_%H%M")
    if args.debug:
